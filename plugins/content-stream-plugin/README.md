...
4. **Domain tool** (staff only):
   ...
   This is **not** a row in **Pages**; it is a plugin route. The slug `content-stream` is registered via `registerPluginReservedSlugs()` in `ContentStreamServiceProvider` so it cannot collide with a content type.
