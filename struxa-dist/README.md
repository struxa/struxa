...
**Note:** `publish.json` does **not** remove `plugins/` from the main git repo. CMS auto-updates and safe FTP ZIPs skip `plugins/` (see repo root `.gitattributes` and `CmsSelfUpdater`). They also skip the **live catalog** under `public/struxa-dist/` (`repo.json`, ZIPs, shards) so a CMS update cannot overwrite StruxaPoint’s registry — production catalog is regenerated by **struxa-admin**. Plugin source can stay in git for development; production sites install plugins from the catalog only.
