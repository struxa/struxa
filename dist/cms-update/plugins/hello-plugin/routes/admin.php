...
return function (App $app, PluginBootContext $ctx): void {
    ...
    $app->group('/admin', function (\Slim\Routing\RouteCollectorProxy $group) use ($ctx, $twig): void {
        $group->get('/hello-plugin', function (Request $request, Response $response) use ($ctx, $twig): Response {
            /** @var array<string, mixed> $cmsUser */
            $cmsUser = $request->getAttribute('cms_user') ?? [];

            return $twig->render($response, '@plugin_hello_plugin/admin/hello.twig', array_merge($ctx->viewData(), [
                'cms_user' => $cmsUser,
                'admin_nav' => 'extensions_plugins',
            ]));
        })->setName('plugin.hello_plugin.admin');
    })->add($middleware);
};
