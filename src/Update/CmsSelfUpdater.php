...
final class CmsSelfUpdater
{
    ...
    private function isMergeExcluded(string $relativeUnix): bool
    {
        ...
        if ($this->isLiveCatalogMergeExcluded($rel)) {
            return true;
        }
    ...
    /**
     * Live StruxaPoint catalog (repo.json, ZIPs, shards) is generated by struxa-admin on the server.
     * CMS updates must not overwrite it from the git snapshot.
     */
    private function isLiveCatalogMergeExcluded(string $rel): bool
    {
        if (str_starts_with($rel, 'public/struxa-dist/zips/')) {
            return true;
        }
        if (str_starts_with($rel, 'public/struxa-dist/catalog/')) {
            return true;
        }

        return in_array($rel, [
            'public/struxa-dist/repo.json',
            'public/struxa-dist/repo-summary.json',
            'public/struxa-dist/publish.json',
            'public/struxa-dist/news.json',
        ], true);
    }
